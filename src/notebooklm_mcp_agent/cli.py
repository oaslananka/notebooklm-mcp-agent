...
# The login command invokes fixed Python modules without a shell.
...
from notebooklm_mcp_agent.logging_setup import configure_logging
from notebooklm_mcp_agent.server import create_server
from notebooklm_mcp_agent.tools.admin import build_health, build_version_info
from notebooklm_mcp_agent.tools.common import to_plain
from notebooklm_mcp_agent.transport.stdio_runner import run_stdio

app = typer.Typer(help="NotebookLM MCP server.")


def _version_line() -> str:
    parts = [f"notebooklm-agent {__version__}"]
    commit = os.getenv("NOTEBOOKLM_MCP_COMMIT_SHA") or os.getenv("GITHUB_SHA")
    build_date = os.getenv("NOTEBOOKLM_MCP_BUILD_DATE") or os.getenv("BUILD_DATE")
    if commit:
        parts.append(f"commit={commit}")
    if build_date:
        parts.append(f"build_date={build_date}")
    return " ".join(parts)
...
async def _openapi_schema(request: Request) -> JSONResponse:
    ...
    from notebooklm_mcp_agent.openapi import OPENAPI_SCHEMA  # noqa: PLC0415
    ...
    if settings.auth_mode is AuthMode.BEARER:
...
async def _ai_plugin_manifest(request: Request) -> JSONResponse:
    ...
    manifest = {
        ...
        "name_for_model": "notebooklm_mcp_agent",
        ...
        "contact_email": "169144131+oaslananka@users.noreply.github.com",
        "legal_info_url": "https://github.com/oaslananka/notebooklm-mcp-agent/blob/main/LICENSE",
    }
...
async def _tool_action(request: Request) -> JSONResponse:
    ...
    from notebooklm_mcp_agent.openapi import resolve_tool_name  # noqa: PLC0415
...
def _notebooklm_default_auth_file() -> Path:
    from notebooklm_mcp_agent.backend.client import (  # noqa: PLC0415
...
def _http_app(settings: Settings | None = None) -> Starlette:
    from notebooklm_mcp_agent.auth.middleware import AuthMiddleware  # noqa: PLC0415
    ...
    if resolved.auth_mode is AuthMode.GITHUB_OAUTH:
        from notebooklm_mcp_agent.auth.oauth import GitHubOAuthHandler  # noqa: PLC0415
...
def login(
    ...
    if dry_run:
        if importlib.util.find_spec("notebooklm") is None:
            typer.echo("notebooklm module not found; install notebooklm-mcp-agent first.", err=True)
            raise typer.Exit(1)
        if importlib.util.find_spec("playwright") is None:
            typer.echo("playwright module not found; install notebooklm-mcp-agent first.", err=True)
            raise typer.Exit(1)
    ...
    try:
        subprocess.run(browser_command, check=True)  # noqa: S603  # nosec B603
...
def doctor(
    offline: Annotated[
        bool,
        typer.Option("--offline", help="Run only local diagnostics."),
    ] = False,
) -> None:
    """Print local environment diagnostics."""
    _ = offline
    settings = Settings()
    from notebooklm_mcp_agent.backend.client import resolve_auth_source  # noqa: PLC0415
    from notebooklm_mcp_agent.backend.exceptions import BackendAuthError  # noqa: PLC0415
