...
from notebooklm_mcp_agent import cli as cli_module
from notebooklm_mcp_agent.cli import _http_app, app
from notebooklm_mcp_agent.config import AuthMode, Settings, TransportMode
...
def test_cli_prints_version(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.delenv("GITHUB_SHA", raising=False)
    monkeypatch.delenv("NOTEBOOKLM_MCP_COMMIT_SHA", raising=False)
    monkeypatch.delenv("BUILD_DATE", raising=False)
    monkeypatch.delenv("NOTEBOOKLM_MCP_BUILD_DATE", raising=False)

    result = CliRunner().invoke(app, ["--version"])

    assert result.exit_code == 0
    assert result.stdout.strip() == f"notebooklm-agent {__version__}"


def test_cli_prints_version_metadata(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setenv("NOTEBOOKLM_MCP_COMMIT_SHA", "abc123")
    monkeypatch.setenv("NOTEBOOKLM_MCP_BUILD_DATE", "2026-05-19T00:00:00Z")

    result = CliRunner().invoke(app, ["--version"])

    assert result.exit_code == 0
    assert result.stdout.strip() == (
        f"notebooklm-agent {__version__} commit=abc123 build_date=2026-05-19T00:00:00Z"
    )
...
def test_cli_doctor_reports_missing_custom_auth_file(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setenv("NOTEBOOKLM_MCP_NOTEBOOKLM_AUTH_FILE", "missing-custom-auth.json")
...
def test_cli_transport_commands_are_present() -> None:
    ...
    serve = runner.invoke(app, ["serve", "--host", "127.0.0.1", "--dry-run"])
...
def test_cli_login_dry_run_reports_missing_notebooklm(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.cli.importlib.util.find_spec", fake_find_spec)
...
def test_cli_login_runs_module_command_with_storage_path(
    ...
    monkeypatch.setenv("NOTEBOOKLM_MCP_NOTEBOOKLM_AUTH_FILE", str(auth_file))
    monkeypatch.setattr("notebooklm_mcp_agent.cli.subprocess.run", fake_run)
...
def test_cli_login_syncs_newer_notebooklm_profile_storage(
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.cli.subprocess.run", fake_run)
    monkeypatch.setattr(
        "notebooklm_mcp_agent.cli._notebooklm_default_auth_file",
...
def test_cli_notebooklm_default_auth_file_delegates_to_backend(
    ...
    monkeypatch.setattr(
        "notebooklm_mcp_agent.backend.client._notebooklm_default_auth_file",
        lambda: profile_storage,
    )
...
def test_cli_login_reports_auth_sync_filesystem_error(
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.cli.shutil.copy2", fake_copy2)
...
def test_cli_login_ignores_incomplete_http_auth_environment(
    ...
    monkeypatch.setenv("NOTEBOOKLM_MCP_AUTH_MODE", "bearer")
    monkeypatch.delenv("NOTEBOOKLM_MCP_BEARER_TOKEN", raising=False)
    monkeypatch.setenv("NOTEBOOKLM_MCP_NOTEBOOKLM_AUTH_FILE", str(auth_file))
    monkeypatch.setattr("notebooklm_mcp_agent.cli.subprocess.run", fake_run)
...
def test_cli_serve_starts_uvicorn(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.cli.uvicorn.run", fake_run)
...
def test_cli_serve_preserves_environment_defaults(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("NOTEBOOKLM_MCP_HTTP_HOST", "127.0.0.2")
    monkeypatch.setenv("NOTEBOOKLM_MCP_HTTP_PORT", str(OVERRIDE_HTTP_PORT))
    monkeypatch.setattr("notebooklm_mcp_agent.cli.uvicorn.run", fake_run)
...
def test_http_health_uses_preconfigured_settings() -> None:
    ...
    assert response.json()["auth_mode"] == "bearer"
...
def test_cli_stdio_invokes_runner(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.cli.run_stdio", fake_run_stdio)
...
def test_cli_version_command_prints_version(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.delenv("GITHUB_SHA", raising=False)
    monkeypatch.delenv("NOTEBOOKLM_MCP_COMMIT_SHA", raising=False)
    monkeypatch.delenv("BUILD_DATE", raising=False)
    monkeypatch.delenv("NOTEBOOKLM_MCP_BUILD_DATE", raising=False)
...
def test_module_entrypoint_runs_cli(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr("sys.argv", ["python -m notebooklm_mcp_agent", "--version"])

    try:
        runpy.run_module("notebooklm_mcp_agent.__main__", run_name="__main__")
