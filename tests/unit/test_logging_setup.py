...
from notebooklm_mcp_agent.config import LogFormat, Settings
from notebooklm_mcp_agent.logging_setup import configure_logging, get_logger


def test_configure_logging_emits_json(capsys: CaptureFixture[str]) -> None:
    configure_logging(Settings(log_format=LogFormat.JSON))

    get_logger("notebooklm_mcp_agent.tests").info("core_started", component="logging")
...
def test_get_logger_returns_structlog_bound_logger() -> None:
    logger = get_logger("notebooklm_mcp_agent.tests")
