...
from notebooklm_mcp_agent.cli import _http_app
from notebooklm_mcp_agent.config import AuthMode, Settings, TransportMode
from notebooklm_mcp_agent.openapi import OPENAPI_SCHEMA, TOOL_SPECS
...
def test_openapi_tool_action_invokes_mcp_tool() -> None:
    ...
    assert response.status_code == HTTP_OK
    assert response.json()["name"] == "notebooklm-mcp-agent"
