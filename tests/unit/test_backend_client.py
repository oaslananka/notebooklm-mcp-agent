...
import notebooklm_mcp_agent.backend.client as client_module
from notebooklm_mcp_agent.backend.client import (
    AuthSource,
    NotebookLMBackend,
    _list_notes_compat,
    resolve_auth_source,
...
def test_auth_file_state_reports_missing_readable_directory_and_os_errors(
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.backend.client.os.access", lambda path, mode: False)
...
async def test_default_client_factory_sets_notebooklm_auth_json_env(
    ...
    monkeypatch.setattr(
        "notebooklm_mcp_agent.backend.client.NotebookLMClient",
        FakeNotebookLMClientClass,
    )
