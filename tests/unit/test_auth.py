...
def _settings(mode: AuthMode = AuthMode.BEARER) -> Settings:
...
class TestTokenValidator:
    ...
    def test_timing_safe_comparison(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "notebooklm_mcp_agent.auth.token.secrets.compare_digest",
            fake_compare_digest,
        )
        validator = TokenValidator(_settings())
        request = _request(headers=[(b"authorization", b"Bearer secret-token")])
...
class TestAuthMiddleware:
    ...
    def test_auth_mode_token_requires_valid_token(self) -> None:
        ...
        assert missing.headers["www-authenticate"] == 'Bearer realm="notebooklm-agent"'
        assert valid.status_code == HTTP_OK
    ...
    def test_invalid_origin_is_rejected(self) -> None:
        ...
        with TestClient(_app(settings)) as client:
            response = client.get("/mcp", headers={"Origin": "https://evil.example.test"})

        assert response.status_code == HTTP_FORBIDDEN

    def test_valid_origin_is_allowed(self) -> None:
        settings = Settings(
            auth_mode=AuthMode.BEARER,
            bearer_token=SecretStr("secret-token"),
            base_url="https://agent.example.test",
