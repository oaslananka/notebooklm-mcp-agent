...
from notebooklm_mcp_agent.auth.oauth import (
    SESSION_COOKIE,
    GitHubOAuthHandler,
    _allowed_users,
    _append_query,
...
def _settings(tmp_path: Any, *, allowed_users: str = "oaslananka") -> Settings:
    return Settings(
...
def test_oauth_helpers(tmp_path: Any) -> None:
    ...
    assert _allowed_users(_settings(tmp_path)) == {"oaslananka"}
...
async def test_login_redirects_to_github(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    monkeypatch.setattr("notebooklm_mcp_agent.auth.oauth.AsyncOAuth2Client", FakeOAuthClient)
...
def test_oauth_register_creates_public_client(tmp_path: Any) -> None:
    ...
    assert payload["client_id"].startswith("notebooklm-agent-")
...
def test_oauth_authorize_rejects_missing_and_invalid_request_params(tmp_path: Any) -> None:
    ...
    with _oauth_client(handler) as client:
        client_id = _register_client(client, redirect_uri)
        missing = client.get("/oauth/authorize", params={"response_type": "bearer"})
...
async def test_callback_rejects_missing_access_token(
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.auth.oauth.AsyncOAuth2Client", EmptyTokenOAuthClient)
    handler = GitHubOAuthHandler(_settings(tmp_path))
...
async def test_github_user_parses_response(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    ...
    monkeypatch.setattr("notebooklm_mcp_agent.auth.oauth.httpx.AsyncClient", FakeAsyncClient)
    handler = GitHubOAuthHandler(_settings(tmp_path))

    assert await handler._github_user("bearer") == {"login": "oaslananka"}
